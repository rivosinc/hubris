...
/// return to user. This should be done "on our way out" to user code:
/// Example: towards the end of the syscall routine or before starting first
/// task.
///
...
pub unsafe fn activate_next_task(task: &mut Task) {
    crate::arch::apply_memory_protection(task);
    // Safety: our contract above is sufficient to ensure that this is safe.
    unsafe {
        crate::arch::set_current_task(task);
    }
